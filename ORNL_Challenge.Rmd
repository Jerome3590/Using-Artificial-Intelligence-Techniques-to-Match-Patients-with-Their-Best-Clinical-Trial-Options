...
#Create 'nlp_in' folder to store files under existing 'data' folder
...
```{python NLP Processor Stage I All Files}
...
def run_nlp(doc,fname):
    doc = nlp(doc,fname)
    with open('data//nlp_out//NLP_output.txt', 'w') as out_file:
        for p in doc._.phrases:
           print("{:.4f} {:5d} {} {} ".format(p.rank, p.count, p.text, fname))
           out_file.write("{:.4f} {:5d} {} {} ".format(p.rank, p.count, p.text, fname))
...
```{python NLP Text Similarity Processor Stage II}
...
corpus = ['Compatibility', 'of', 'systems', 'of', 'linear', 'constraints']
...
def run_nlp2(corpus):
    ...
    with open('data//nlp_out//stage2_output.txt', 'w') as out_file:
        for f in features:
             ...
             distance_euclidean = euclidean_distances(features[0],f)
             ts = np.array_str(distance_euclidean)
             out_file.write(ts)
